...
出題した問題は`history/log.md`に1問ずつ記録する（フォーマットは`history/log.md`内の説明を参照。`SQL/history/log.md`と同じ考え方）。

- **新しいセッションで「APIスタート」と言われたら**、起動確認の後・問題を出す前に、まず`history/log.md`の末尾（直近のエントリ）を確認し、直近のリソース・CRUD操作・パラメータパターン・難易度を把握してから次の問題を決める（同じ組み合わせを連続させない、7章のローテーションルールに従う、前回より少しずつ難易度・パターンを広げる）。
- **問題を出題したら**、その内容を`history/log.md`に追記する（結果は「未回答」）。
- **ユーザーがcurlの実行結果を報告したら**、採点後にそのエントリの「結果」を更新する（✅正解 / ⚠️惜しい / ❌不正解）。
