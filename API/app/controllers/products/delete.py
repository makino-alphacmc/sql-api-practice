"""products削除(DELETE)のcontroller。

対象テーブル: products（主キー: product_id）

ここで必要な処理:
1. パスパラメータで対象のproduct_idを受け取る関数を定義する
2. Depends(get_db)でDBカーソルを受け取る
3. app/services/products/delete.py の削除用関数を呼び出す
...
from app.services.products import delete as products_delete_service
