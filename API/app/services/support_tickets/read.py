...
[単体取得]
1. 主キー（ticket_id）を条件にSELECT文を組み立てる（%sプレースホルダを使う）
2. cur.execute()の後、cur.fetchone()で1行取得して返す
   （見つからなければNoneが返る。404の判定はcontroller側の仕事）
"""


def list_support_tickets(cur, *args, **kwargs):
    ...


def get_support_ticket_by_id(cur, ticket_id: int):
    ...
